...
- 한 커밋은 한 가지 목적. 성격이 다르면(접근성/버그/리팩터/문서) 커밋을 나눈다.
- 제목은 `type: 무엇을` 형태로 평이한 한국어. 사람이 한 줄로 바로 이해되게.
...
```
fix: 접근성 - 검색 모달 focus trap 적용

모달이 열려 있을 때 Tab/Shift+Tab이 모달 안에서만 순환하도록 가둔다.
(외부 라이브러리 없이 작은 훅으로 구현)
```
